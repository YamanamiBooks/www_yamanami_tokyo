...
* p.408  用語索引 多線比例計数管 → 多線比例計数箱
    - （編者追補）電極線を囲んだ計数管を多数線状に配列しchamber(箱)として構成されたMWPC形式のものなので普通は「計数箱」の呼称が使われている。計数管の一形式には違いなくその呼称も使われることがありますが，計数装置を専門とする著者がp.139で意識的に比例計数管と多線比例計数箱を区別して並べているので，文中の通り計数箱としておきたい。
